...
# Abstract
Normally the credit scoring algorithm takes the linear generalized model as the calculating method of probability of credit bankruptcy. From records of credit history of normal information, by taking age and income as groups, we explore the correlation between the likelihood of having delinquent between groups. The finding is that people in the group of ages lower than 45 have higher possibility of having delinquent is...
...
By visualizing the dataset and taking a look at the summary of the each columns. I removed the outliers having age smaller than 16. Also, for the number recording the times people past the due dates, I removed the number larger than the maximum possible times. More detailed progress could be seen from the codes.  

```{r,message=FALSE,warning=FALSE,echo=FALSE}
#partition(skim(tr))
count(tr %>% group_by(SeriousDlqin2yrs))
```
In 144564 lines of people's credit, 6.6% of them have record of esrios delinquent in 2 years time. Among the records, the distribution of age is similar to the normal distribution. Besides age, the distribution of other factors(income, number of times exeeding the due dates 30 days more, number of dependents, balance divided by credit limit) are extreme left skewed. 75% of the people have income lower than 7500, 25% of people have income between 7500 and 3000000. With the maximum of 54 of the balance on credit cards divided by the credit limits, 75% of people is under 2. 

## Model Choice

```{r,message=FALSE,warning=FALSE,echo=FALSE,fig.height=2,fig.width=2}
# Make group by age 
...
```{r,message=FALSE,warning=FALSE,echo=FALSE,fig.height=4,fig.width=3}
...
library("bayesplot")
pred<- posterior_predict(glmer1,iter=1000)
ppc <- ppc_dens_overlay(pred[1:100,], y=tr.s$SeriousDlqin2yrs)
ppc
```
